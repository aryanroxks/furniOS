...
const updatePurchase = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    const downloadPurchasesPDF = asyncHandler(async (req, res) => {
  const { status, vendorId, startDate, endDate } = req.query;
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=purchases_report.pdf"
  );
  ...
  purchases.forEach((p) => {
    doc.text(new Date(p.purchaseDate).toLocaleDateString(), 40);
    doc.text(p.vendorId?.name || "-", 120);
    doc.text(String(p.items.length), 300);
    doc.text(`₹${p.totalAmount}`, 360);
    doc.text(p.status, 430);
    doc.moveDown(0.3);
  });
